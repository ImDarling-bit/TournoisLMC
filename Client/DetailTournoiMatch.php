...
<head>
    ...
    <style>
        body { background: linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.6)), url('assets/img/Image_fond.png'); background-size: cover; background-position: center; background-attachment: fixed; color: #fff; min-height: 100vh; display: flex; flex-direction: column; }
        .tournament-header-banner { background: linear-gradient(rgba(21, 26, 33, 0.2), rgba(21, 26, 33, 0.95)), url('assets/img/rocket-league-cover-680x89.jpg'); background-size: cover; background-position: center; border-radius: 12px 12px 0 0; padding: 40px; margin-top: 20px; border: 1px solid #30363d; border-bottom: none; }
        .tournament-main-card { background-color: rgba(21, 26, 33, 0.95); border: 1px solid #30363d; border-radius: 0 0 12px 12px; padding: 30px; box-shadow: 0 15px 35px rgba(0, 0, 0, 0.5); min-height: 600px; }
        .nav-tabs-custom { background: #0d1117; border-radius: 8px; padding: 5px; border: 1px solid #30363d; margin-bottom: 30px; }
        .nav-tabs-custom .nav-link { border: none; color: #8b949e; font-weight: 600; text-transform: uppercase; font-size: 0.8rem; padding: 10px 20px; transition: 0.3s; }
        .nav-tabs-custom .nav-link:hover { color: #fff; }
        .nav-tabs-custom .nav-link.active { background-color: #21262d; color: #fff; border-radius: 6px; }

        .tournament-bracket-container { display: flex; align-items: stretch; justify-content: center; gap: 40px; padding-bottom: 20px; overflow-x: auto; }
        .round-column { display: flex; flex-direction: column; width: 280px; flex-shrink: 0; }
        .round-header { text-align: center; color: #8b949e; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 20px; border-bottom: 1px solid #30363d; padding-bottom: 10px; height: 40px; flex-shrink: 0; }
        .match-list { display: flex; flex-direction: column; flex: 1; }
        .match-item { display: flex; flex-direction: column; justify-content: center; flex: 1; position: relative; padding: 10px 0; }
        
        .match-card { background-color: #0d1117; border: 1px solid #30363d; border-radius: 8px; padding: 0; box-shadow: 0 4px 6px rgba(0,0,0,0.3); transition: transform 0.2s, border-color 0.2s; position: relative; z-index: 2; width: 100%; margin: 0 !important; }
        .match-card:hover { border-color: #58a6ff; }

        .round-column:not(:last-child) .match-item:nth-child(odd)::after { content: ''; position: absolute; right: -20px; top: 50%; bottom: 0; width: 20px; border-top: 2px solid #58a6ff; border-right: 2px solid #58a6ff; border-top-right-radius: 6px; z-index: 1; }
        .round-column:not(:last-child) .match-item:nth-child(even)::after { content: ''; position: absolute; right: -20px; top: 0; bottom: 50%; width: 20px; border-bottom: 2px solid #58a6ff; border-right: 2px solid #58a6ff; border-bottom-right-radius: 6px; z-index: 1; }
        .round-column:not(:last-child) .match-item:only-child::after { border: none !important; border-top: 2px solid #58a6ff !important; bottom: auto !important; top: 50% !important; border-radius: 0 !important; }
        .round-column:not(:first-child) .match-item::before { content: ''; position: absolute; left: -20px; top: 50%; width: 20px; border-top: 2px solid #58a6ff; z-index: 1; }

        .team-row { display: flex; align-items: center; padding: 8px 12px; border-bottom: 1px solid #21262d; height: 60px; }
        .team-row:last-child { border-bottom: none; }
        .team-logo { width: 25px; height: 25px; border-radius: 50%; object-fit: cover; margin-right: 10px; background-color: #21262d; border: 1px solid #30363d; }
        
        .team-select { flex-grow: 1; background: #161b22; color: #fff; border: 1px dashed #30363d; padding: 5px; border-radius: 5px; font-size: 0.85rem; outline: none; cursor: pointer; transition: 0.2s; appearance: none; -webkit-appearance: none; }
        .team-select:hover { border-color: #58a6ff; background: #21262d; }
        .team-select:focus { border-color: #58a6ff; border-style: solid; }
        .team-select option { background: #0d1117; color: #fff; }
        
        .score-input { width: 45px; background: #161b22; color: #fff; border: 1px solid #30363d; border-radius: 4px; padding: 4px; text-align: center; font-weight: bold; font-size: 0.9rem; outline: none; margin-left: 10px; }
        .score-input:focus { border-color: #58a6ff; }
        
        .save-match-btn { width: 100%; background: #21262d; color: #58a6ff; border: none; padding: 6px; border-top: 1px solid #30363d; font-size: 0.75rem; font-weight: bold; text-transform: uppercase; border-radius: 0 0 8px 8px; cursor: pointer; transition: 0.2s; }
        .save-match-btn:hover { background: #58a6ff; color: #0d1117; }

        .winner .team-select { color: #fff; font-weight: bold; border: 1px solid transparent; background: transparent; pointer-events: none; }
        .winner .score-input { color: #2ecc71; border-color: #2ecc71; pointer-events: none; }
        .loser .team-select { color: #484f58; border: 1px solid transparent; background: transparent; pointer-events: none; }
        .loser .score-input { color: #484f58; border-color: #30363d; pointer-events: none; }
        
        .champion-card { border: 2px solid #f1c40f; box-shadow: 0 0 15px rgba(241, 196, 15, 0.4); text-align: center; padding: 20px; background: linear-gradient(45deg, #0d1117, #1a1e23); }
        .champion-card .team-name { font-size: 1.2rem; color: #f1c40f; display: block; margin-top: 10px; font-weight: bold; }
        
        .btn-generate { background-color: #2ea043; color: white; border: none; padding: 15px 30px; border-radius: 8px; font-weight: bold; font-size: 1.1rem; transition: 0.3s; box-shadow: 0 4px 10px rgba(46, 160, 67, 0.3); }
        .btn-generate:hover { background-color: #238636; transform: translateY(-2px); }
        .btn-generate:disabled { background-color: #21262d; color: #8b949e; cursor: not-allowed; box-shadow: none; transform: none; border: 1px solid #30363d; }
    </style>
</head>
